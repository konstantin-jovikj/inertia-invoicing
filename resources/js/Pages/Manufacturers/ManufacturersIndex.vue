                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex justify-between mb-4">
                            <h2 class="font-bold text-sky-800">Добавувачи / Производители</h2>
                            <div class="flex">
                                <Link href="manufacturer/create" class="mx-4 mt-2 text-5xl hover:text-sky-500 text-slate-500">
                                <AddIcon />
                                </Link>
                            </div>
                        </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 shadow table-auto sm:rounded-lg">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr class="hover:bg-slate-100" v-for="(manufacturer) in manufacturers"
                                    :key="manufacturer.id">
                                    <td class="text-xs text-slate-300">
                                        {{ manufacturer . id }} {{}}
                                    </td>

                                    <td class="font-bold text-emerald-700">{{ manufacturer . name }}</td>
                                    <td class="font-sm italic text-zinc-500">{{ manufacturer . address }}</td>
                                    <td class="">
                                        <span class="text-sky-700">{{ manufacturer . place . zip }}</span>
                                        <span class="px-2">-</span>
                                        <span>{{ manufacturer . place . place }}</span>
                                        <span class="px-2">-</span>
                                        <span class="font-bold text-sky-800">{{ manufacturer . place . country . name }}</span>
                                    </td>
